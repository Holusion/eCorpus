...
import { getKtxVersion } from "../exec.js";
...
export const toktx = function (options: ETC1SOptions | UASTCOptions): Transform {
    ...
    for (let textureIndex = 0; textureIndex< textures.length; textureIndex++){
      ...
      if(typeof options.maxSize === "number" || !srcSize.every(n=>isMultipleOfFour(n)) || texture.getMimeType() === "image/webp"){
        ...
        srcImage = BufferUtils.toView((await encoder.toBuffer()) as any);
        srcExtension = 'png';
      ...
      if (code !== 0) {
        ...
        texture.setImage(await fs.readFile(dstPath) as any).setMimeType('image/ktx2');
        if (texture.getURI()) {
