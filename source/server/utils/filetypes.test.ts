...
import { compressedMime, extFromType, getContentType, getFilename, getMimeType, parseMagicBytes, readMagicBytes } from "./filetypes.js";
...
describe("getFilename", function () {
  it("returns filename from content-disposition", function () {
    expect(getFilename({
      "content-disposition": 'attachment; filename="report.pdf"',
    } as any)).to.equal("report.pdf");
  });

  it("falls back to the content-type extension", function () {
    expect(getFilename({
      "content-type": "image/jpeg",
    } as any)).to.match(/^[A-Za-z0-9_-]{12}\.jpeg$/);
  });

  it("falls back to the content-type extension when content-disposition is unsafe", function () {
    expect(getFilename({
  ...
  [
    {
      ...
      header: 'attachment; filename="../secret.txt"',
    },
    {
      name: "nested relative traversal",
      header: 'attachment; filename="../../var/data/archive.zip"',
    },
    {
    ...
    },
    {
      ...
      header: 'attachment; filename="C:\\Windows\\System32\\drivers\\etc\\hosts"',
    },
    {
      name: "windows parent traversal",
      header: 'attachment; filename="..\\..\\AppData\\secret.txt"',
    },
  ].forEach(({ name, header }) => {
    it(`rejects malicious ${name}`, function () {
      expect(getFilename({
