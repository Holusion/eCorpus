...
describe("types compatibility", function(){
  describe("IDocument", function(){
    it("is compatible with IDocument from DPO-Voyager", function(){
      let diagnostics = compare({
      ...
      expect(diagnostics.length, `Interfaces are not compatible:\n\t${diagnostics.join("\n\t")}`).to.equal(0);
    });
  });
});
...
export function compare(i1:InterfaceSource, i2:InterfaceSource):string[] {
  ...
  if (!interface1Decl) {
    throw new Error(`Interface '${i1.name}' not found.`);
    return diagnostics;
  }
  if (!interface2Decl) {
    throw new Error(`Interface '${i2.name}' not found.`);
  }
  ...
  // Check if type1 is assignable to type2 AND type2 is assignable to type1
  const isCompatibleForward = checker.isTypeAssignableTo(type1, type2);
  const isCompatibleBackward = checker.isTypeAssignableTo(type2, type1);


  if(!isCompatibleForward){
    diagnostics.push(...explain(
      checker,
      {type:type1, declaration: interface1Decl, prefix: `${prune(i1.file, i2.file)}:${i1.name}`},
      {type:type2,  declaration: interface2Decl, prefix: `${prune(i2.file, i1.file)}:${i2.name}`},
    ));

  }
  if(!isCompatibleBackward){
    diagnostics.push(...explain(
      checker,
      {type:type2,  declaration: interface2Decl, prefix: `${prune(i2.file, i1.file)}:${i2.name}`},
      {type:type1, declaration: interface1Decl, prefix: `${prune(i1.file, i2.file)}:${i1.name}`},
  ...
  // console.log(i1.name, type1);
  // console.log(i2.name, type2);
  return diagnostics;
}
...
function explain(checker: InternalTypeChecker, from:TypeRef, to:TypeRef):string[]{
  ...
  for(let property of from.type.getProperties()){
    ...
    if( typeof toProperty === "undefined"){
      diagnostics.push(`Property ${property.getName()} is present in ${from.prefix} but absent in ${to.prefix}`);
      continue;
    }
      ...
      if (fromPropType.flags & ts.TypeFlags.Object && toPropType.flags & ts.TypeFlags.Object) {
        ...
        if(isSourceArray && isTargetArray){
          ...
          if(!checker.isTypeAssignableTo(fromElementType, toElementType)){
            ...
            console.log(`Unable to explain why ${from.prefix}.${fromProp.getName()} is not assignable to ${to.prefix}.${toProp.name}`)
          }
          ...
          diagnostics.push(`Incompatible types: ${from.prefix}.${fromProp.getName()} is an ${isSourceArray?"array":"object"} but ${to.prefix}.${toProp.getName()} is an ${isTargetArray? "array": "object"}`)
        }
      }else{
        ...
        if(fromPropTypeName != toPropTypeName){
          diagnostics.push(`Incompatible types: ${from.prefix}.${fromProp.getName()} is a ${fromPropTypeName} ${to.prefix}.${fromProp.getName()} is a ${toPropTypeName}`);
          ...
          diagnostics.push(`Interface ${from.prefix}.${fromProp.getName()} is missing value${1 <missing.length? "s":""} ${missing.join(", ")} from ${to.prefix}.${toProp.getName()}`);
        }else{
          diagnostics.push(fromPropType.flags.toString(2));
          diagnostics.push(`Incompatible types with same names found : ${fromPropTypeName} at ${from.prefix}.${fromProp.getName()}, ${to.prefix}.${toProp.getName()}`);
        }
      }
    }else{
      //console.log(`${from.prefix}.${fromProp.getName()} is assignable to ${to.prefix}.${toProp.name}`)
    }
  }
  return diagnostics;
}
