...
export function getFilename(headers: Request["headers"]): string | undefined {
  const dispositionHeader = typeof headers["content-disposition"] === "string" ? headers["content-disposition"] : undefined;
  const disposition = dispositionHeader ? contentDisposition.parse(dispositionHeader) : null;
