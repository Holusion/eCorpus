...
import { TaskDefinition, TaskTypeData } from "../../../tasks/types.js";
import { createWriteStream } from "node:fs";
import { pipeline } from "node:stream/promises";

export function isUploadTask(t:TaskDefinition<any>) : t is TaskDefinition< TaskTypeData<"userUploads">>{
...
export async function putUploadTask(req: Request, res: Response){
  ...
  if(!contentRange){
    ...
    taskScheduler.setTaskStatus(task.task_id, "pending");
    res.status(201);
    return   res.format({
      "text/plain": ()=>{
        res.send("Created")
