...
describe("GET /tasks/:id", function(){
  ...
  it("returns task with logs", async function(){
    ...
    expect(body.task).to.have.property("type", "test");
  });
  ...
  describe("permissions", function(){
    ...
    describe("scene-based access", function(){
      ...
      this.beforeAll(async function(){
        scene_id = await vfs.createScene("task-get-test");
        // Make the scene private so default access doesn't grant read
        await request(this.server).patch(`/scenes/task-get-test`)
        .auth("alice", "12345678")
        .send({default_access: "none", public_access: "none"})
        .expect(200);
      });

      it("user with read access to the scene can access the task", async function(){
      ...
      it("user without scene access cannot access the task", async function(){
        await userManager.grant(scene_id, other.uid, "none");
        const task = await taskScheduler.create({scene_id, user_id: null, type: "test", data: {}});
        const res = await request(this.server).get(`/tasks/${task.task_id}`)
        .auth("charlie", "12345678");
