...
describe("COPY /scenes/:name", function(){
  ...
  it("can COPY a document", async function(){
    ...
    let {ctime:t1, id, generation, mtime,...src} = await vfs.getDoc(scene_id);
    ...
    .expect(201);
    let {ctime:t2, id:id2, generation:g2, mtime:mt2, ...new_doc} = await expect(vfs.getDoc(scene_id), "file should still be here").to.be.fulfilled;
    expect(id2).not.to.equal(id);
    expect(g2).to.equal(4);
    expect(mt2).to.be.instanceOf(Date);
