...
import { toktx } from '../../utils/gltf/toktx.js';

import {type ProcessFileParams, type TaskHandlerParams, type ITaskLogger, FileArtifact} from "../types.js";
...
export async function optimizeGlb({task: {task_id, data:{fileLocation, preset: presetName}}, context:{ vfs, logger, config }}:TaskHandlerParams<ProcessFileParams>):Promise<FileArtifact>{
  if(!fileLocation) throw new Error(`A file is required for this task to run`);

  const resize = true; //Might make it configurable
  ...
  const inputFile = vfs.absolute(fileLocation);
  ...
  //FIXME : Don't touch geometry if it's already compressed using draco or meshopt?
  ...
  logger.log("Output file uses extensions:", root.listExtensionsUsed().map(e=>e.extensionName));
  await io.write(outputFile, document);

  //Resize textures
  return {fileLocation: vfs.relative(outputFile)};
};
