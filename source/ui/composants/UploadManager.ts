...
interface UploadOperation{
  ...
  files?:string[];
  scenes?: {name: string, action: "create"|"update"|"error"}[];
...
export default class UploadManager extends LitElement{
  ...
  async finalizeUpload(task: UploadOperation) :Promise<any>{
    const res = await fetch(`/tasks/task/${task.task_id}`, {
      method: "GET",
  ...
  private processUploads(){
    ...
    xhr.onload = ()=>{
      if (299 < xhr.status) {
        ...
        console.warn("Verify headers!");
        this.finalizeUpload(task).then((body)=>{
          console.debug("Finalized upload task. Parsed content :", body);
          if(!body.output || !Array.isArray(body.output.scenes) || !Array.isArray(body.output.files)){
            console.warn("Unexpected format for scene output:", body);
            Notification.show("Unexpected shape of uploaded data.", "error", 15000);
            setError({code: 500, message: "Failed to finalize upload"});
          }else{
            update({active: false, progress: task.total, done: true, ...body.output});
          }
        }, setError);
      }else{
    ...
    let url = new URL(`/tasks/artifacts/${task.task_id}`, window.location.href);
  ...
  private renderUploadItem = (u:UploadOperation)=>{
    const onActionClick = (ev: MouseEvent)=>{
      ...
      ev.stopPropagation();
      if(u.error || u.done){
        u.task_id && fetch(`/tasks/task/${u.task_id}`, {method: "DELETE"})
    ...
    if(u.error){
      state = "error";
      stateText = "⚠";
      progress= `${u.error.code?`[${u.error.code}] `:""}${u.error.message}`;
    }else if(u.done){
      state = "done";
      stateText = "✓";
  ...
  private renderSceneCreationForm(){
    ...
    return html`<slot name="upload-form" ?disabled=${has_pending_uploads}></slot>
      <div class="submit-container" style="display:flex; justify-content: end; gap: .5rem;">
        ${(()=>{
          if(has_pending_uploads) return html`<span class="loader" style="width: 34px; height: 34px; --color-loader: var(--color-secondary);"></span>`
          else if(has_errors) return html`<slot name="upload-errors">Some uploads have failed</slot>`
          else if(this.uploads.length) return html`<slot name="submit" ?disabled=${has_pending_uploads}><button>Submit</button></slot>`
          else return null;
        })()}
